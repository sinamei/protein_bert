...
def plot_2d_interactive(embeddings_2d, labels, title, unique_label_name, save_path):
    ...
    Parameters:
      ...
      unique_label_name: Name of the label (e.g., "GO IDs").
...
def main():
    ...
    # The CSV is expected to have a "Sequence" column, a "supfam_ids" column, and a "go_ids" column.
    ...
    if "supfam_ids" in df.columns:
        ...
        # Filter the dataset for entries that contain the most frequent SUPFAM.
        ...
        if df_filtered.empty:
            ...
            return
        # Use the "Sequence" column from the filtered dataset.
        sequences = df_filtered["Sequence"].tolist()
        # Now use GO IDs for coloring.
        if "go_ids" in df_filtered.columns:
            labels = df_filtered["go_ids"].apply(
    ...
    plot_2d_interactive(
        ...
        save_path="proteinbert_tsne_supfam_by_GO.html"
    )
    try:
    ...
    #     unique_label_name=label_name,
    #     save_path="proteinbert_umap_supfam_by_GO.html"
    # )
    # try:
