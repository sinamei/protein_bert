...
def main():
    ...
    # Load the test CSV file (expects a "Sequence" column and a "supfam_ids" column).
    test_csv_path = 'test_data_annotations.csv'
    ...
    # For evaluation by SUPFAM IDs, if the CSV contains a "supfam_ids" column,
    # extract a label for each protein. If multiple SUPFAM IDs are present (comma-separated),
    # here we take the first one.
    if "supfam_ids" in df.columns:
        labels = df["supfam_ids"].apply(
    ...
    plot_2d_interactive(embeddings_2d_tsne, labels=labels,
                        title="2D TSNE of ProteinBERT Global Embeddings (by SUPFAM IDs)",
                        save_path="proteinbert_supfam_tsne_2d.html")

    # --- Compute Silhouette Score ---
    try:
        score = silhouette_score(embeddings_2d_tsne, labels)
        print(f"Silhouette score (SUPFAM IDs, TSNE): {score:.3f}")
    except Exception as e:
        print(f"Could not compute silhouette score for SUPFAM IDs (TSNE): {e}")
